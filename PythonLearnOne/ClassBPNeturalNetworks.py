...
class BPNeturalNetworks(object):
    ...
    def SGD(self, training_data, test_data=None):
        ...
        for j in range(self.epochs):
            ...
            for mini_batch in mini_batches:
                self.SGD_mini_batch(mini_batch)
    ...
    def SGD_mini_batch(self, mini_batch):
        '''
    ...
    def backprop(self, x, y):
        ...
        # 上一层网络的输出作为下一层的输入
        activation = np.matrix(x).transpose()  # 一个样本的全部特征数据
...
def sigmoid_prime(z):
    """sigmoid的求导."""
    return np.dot(sigmoid(z).transpose(), (1 - sigmoid(z)))
