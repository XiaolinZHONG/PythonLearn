...
class WOE:
    ...
    def check_target_binary(self, y):
        ...
        :return:
        '''
        y_type = type_of_target(y) # 通过 sklearn中的函数来确定 target 是二分类
    ...
    def discrete(self, x):
        ...
        切分数据成5个 BIN:首先使用 numpy 自带的工具获得相应的切分阈值
        :param x: 1-D numpy array：一维数组，后面可以优化为 pandas的 series形式
        :return: discreted 1-D numpy array：返回值是切分后的数组
        '''
        res = np.array([0] * x.shape[-1], dtype=int)
        for i in range(5):
            point1 = stats.scoreatpercentile(x, i * 20)
            point01 = np.percentile(x,i*20)
            # 上面的语句可以直接扩展为2维的形式 详情查看 np 的文档
            point2 = stats.scoreatpercentile(x, (i + 1) * 20)
            point02 = np.percentile(x, (i+1) * 20)
            x1 = x[np.where((x >= point1) & (x <= point2))]
            mask = np.in1d(x, x1) # 选择出来符合 X1 的条件中的 X
            res[mask] = (i + 1) #对这一部分的值进行赋值
        return res
...
X=np.array([0,1,2,3,4,6,12,40,12,90])
f=WOE.discrete(-20,X)
print(f)